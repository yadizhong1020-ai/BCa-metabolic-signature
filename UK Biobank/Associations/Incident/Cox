...
nb_cpus = 20
dpath = r'D:\ZYD\R\meta\biobank\incident'

tgt_df = pd.read_csv(os.path.join(dpath, 'IncidentDiseaseTable.csv'))
tgt_lst = tgt_df.Name.tolist() 

nmr_df = pd.read_csv(os.path.join(dpath, 'NMR_Preprocessed.csv'))
nmr_f_lst = nmr_df.columns.tolist()[1:]
cov_df = pd.read_csv(os.path.join(dpath, 'Covariates.csv'))
...
for tgt_name in tqdm(tgt_lst):
    tgt_file = os.path.join(dpath, tgt_name + '.csv')
    ...
    try:
        ...
        if tmp_df.target_y.sum()>=50:
            ...
            tgt_out_df.to_csv(os.path.join(dpath, tgt_name + '_results.csv'), index=False)
        else:
...
bad_df.to_csv(os.path.join(dpath, 'bad_tgt_all.csv'), index=False)
