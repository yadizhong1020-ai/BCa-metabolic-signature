...
run_sex_analysis <- function(full_df, sex_code, sex_name, out_file, nmr_cols, covariates) {
  cat(sprintf("\n--- Running Analysis for %s (Sex == %d) ---\n", sex_name, sex_code))
  
  # Filter: Sex
  if ("Sex" %in% names(full_df)) {
    n_before <- nrow(full_df)
    df <- full_df[full_df$Sex == sex_code, ]
    cat(sprintf("Filtered Sex == %d: %d -> %d\n", sex_code, n_before, nrow(df)))
  } else {
    warning("Sex column not found, skipping Sex filter.")
    df <- full_df
  }
...
main <- function() {
  ...
  # Define covariates
  covariates <- c("Age", "TDI", "BMI", "Smoke", "Statin", "FastingTime")
  ...
  # 3. Run Analysis for Each Sex
  # Sex 1 = Female
  run_sex_analysis(df, 1, "Female", "Cox_Analysis_Results_Female_R.csv", nmr_cols, covariates)
  
  # Sex 0 = Male
  run_sex_analysis(df, 0, "Male", "Cox_Analysis_Results_Male_R.csv", nmr_cols, covariates)
}
